...
#sub-agent: planner
MODEL = os.getenv("MODEL", "MODEL=gemini-2.5-flash")
...
blog_planner = Agent(
   ...
   instruction="""
You are a technical content strategist for engineers looking for a job as data science students. Produce a clear Markdown outline with:
- Title
- Short intro
...
MODEL = os.getenv("MODEL", "gemini-2.5-flash")  # pin a specific model, not -latest

RETRY_CONFIG = types.GenerateContentConfig(
    http_options=types.HttpOptions(
        retry_options=types.HttpRetryOptions(
            attempts=5,          # how many times to retry
            initial_delay=1,     # wait 1s before first retry
            exp_base=2,          # then 2s, 4s, 8s... exponential backoff
            max_delay=30,
